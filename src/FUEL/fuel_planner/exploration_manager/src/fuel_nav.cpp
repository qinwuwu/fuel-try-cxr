...
void Ctrl::control(const ros::TimerEvent&)
{
   ...
   // if(receive)
    ...
    // === 1. 使用Ego-Planner输出的速度作为主指令（推荐）===
    //double cmd_vel_x = ego_vel_x;
    //double cmd_vel_y = ego_vel_y;
    ////double cmd_vel_z = ego_vel_z;

    // === 2. 或者：使用位置误差反馈（P控制器）===
    // double k_p = 0.5;  // 增益调小
    // double cmd_vel_x = k_p * (ego_pos_x - position_x);
    // double cmd_vel_y = k_p * (ego_pos_y - position_y);
    // double cmd_vel_z = k_p * (ego_pos_z - position_z);

    // === 3. 速度限幅（必须加！）===
    //double max_vel_xy = 2.0;  // 限制水平速度不超过2m/s
    //double max_vel_z = 1.0;   // 垂直速度限制

   //// double hor_speed = sqrt(cmd_vel_x*cmd_vel_x + cmd_vel_y*cmd_vel_y);
    //if (hor_speed > max_vel_xy) {
    //    cmd_vel_x = cmd_vel_x / hor_speed * max_vel_xy;
    //    cmd_vel_y = cmd_vel_y / hor_speed * max_vel_xy;
   // }
    
    //cmd_vel_z = std::max(-max_vel_z, std::min(cmd_vel_z, max_vel_z));

    // === 4. 设置目标速度和偏航 ===
   //current_goal.velocity.x = cmd_vel_x;
   // current_goal.velocity.y = cmd_vel_y;
   // current_goal.velocity.z = cmd_vel_z;
   // current_goal.yaw = ego_yaw;

   // ROS_INFO("发布速度: [%.2f, %.2f, %.2f] m/s, yaw=%.2f", 
    //         cmd_vel_x, cmd_vel_y, cmd_vel_z, ego_yaw);
    //}
