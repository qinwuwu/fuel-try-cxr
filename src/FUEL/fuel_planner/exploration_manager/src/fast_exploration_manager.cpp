...
int FastExplorationManager::planExploreMotion(
 ...
 // 更改查错
  ...
  ROS_INFO("Planning path from [%f, %f, %f] to [%f, %f, %f]",
           pos.x(), pos.y(), pos.z(), next_pos.x(), next_pos.y(), next_pos.z());

  // 检查路径规划器是否初始化
  if (!planner_manager_) {
    ROS_ERROR("Planner manager is not initialized.");
    return FAIL;
  }
  if (!planner_manager_->path_finder_) {
    ROS_ERROR("Path finder is not initialized.");
    return FAIL;
  }

  // 检查目标点是否在地图范围内
  // 检查 next_pos 是否在地图内，且不是障碍物（自由空间）
  bool in_map = planner_manager_->edt_environment_->sdf_map_->isInMap(next_pos);
  int occupancy = planner_manager_->edt_environment_->sdf_map_->getOccupancy(next_pos);

  // 有效条件：在地图内 + 占据状态为0（自由空间）
  if (!in_map || occupancy != 0) {
   ROS_WARN("next_pos invalid: in_map=%d, occupancy=%d (0=free, 1=obstacle, -1=unknown)", 
             in_map, occupancy);
    // 此处添加无效处理逻辑（如返回失败、重新选择目标点等）
    return FAIL;  // 根据你的业务逻辑决定是否返回
  }
